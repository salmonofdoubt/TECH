...
func getImages() ([][]pixel, error) {
	...
	for _, path := range paths {
		go func(p string) { //starts goroutine, executes anonymous function
			pixels := getPixels(p)

			mu.Lock() //bit of locking since now multiple cpu try to get to it
			{
			...
			wg.Done()
		}(path) // the momment it had been defined, we are calling it,end of scope of that func
	}
	wg.Wait()

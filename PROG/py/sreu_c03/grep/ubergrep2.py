...
class Args(object): pass									# purpose: handy way of passing things

def ParseArgs(argv):
	print '-- ParseArgs here --'
	args = Args()														# so args becomes an object
	print 'Object of ParseArgs: ', sys.getsizeof(args)
	option_args = 'ABf'
	nonoption_args = 'ncwlLqrvxhHiFoz'
	for arg in nonoption_args:
		setattr(args, arg, False)							# setattr(x, 'foobar', 123) is
			...
			for line_no, line in enumerate(GenLines(f, args)):
				...
				if matches_found:
					...
					if trailing:
						...
						trailing -= 1
		if not args.q:									#still in for loop
